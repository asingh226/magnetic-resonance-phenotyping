#!/usr/bin/env python
# edited by Vansh to match my file structure
...
# pass the paths for the image_mask_name csv, name of output file, name of log_file, path to parameters 
def main(image_mask_names, outputFileName, roi, log_name, parameters, param_type, image_type, radiomics_data_folder='radiomics_data'):
  outPath = r''
  # current convention requires that the columns of the images and masks be labeled "Image" and "Mask" respectively
  inputCSV = image_mask_names
# currently output file is placed one directory out and then back into a directory name radiomics_data, probably best to just define the location path more explicitly
  outputFilepath = os.path.join('..', radiomics_data_folder, param_type, image_type, roi, 'PyRadiomics_raw_data', outputFileName+'.csv')
  progress_filename = os.path.join(outPath, log_name+'.txt')
  params = parameters
...
if __name__ == '__main__':
  main(image_mask_names, outputFileName, roi, log_name, parameters, param_type, image_type, radiomics_data_folder='radiomics_data')
